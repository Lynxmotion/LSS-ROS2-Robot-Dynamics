...
#if 0
void Model::setBaseHeight(double _standingHeight) {
    baseHeight = _standingHeight;
}
#endif

void Model::balance(State& state) {
    balance(state, state);
}
...
void Model::balance(State& state, const ContactState& contacts) {
    ...
    // adjust pitch based on standing height
    p = 0.15 + 0.15 * std::cos(30 * state.baseHeightFromFloor());

    // move the base over the Center-of-Pressure
    KDL::Vector base_to_current_CoM(base.p - state.CoM);

    KDL::Vector target_base_location_vec(
            /* try to center CoM over CoP
             * - note we must adjust for the position of CoM relative to base link origin
             */
            base_to_current_CoM.x() + state.CoP.x(),
            base_to_current_CoM.y() + state.CoP.y(),

            /* place base at desired standing height
             */
            base.p.z()
    );
    KDL::Frame target_base_location(base.M, target_base_location_vec);

    if(interactingSegment != base_link) {
        if(!state.pointInSupport(base.p)) {
            // slowly move back to within support
            double dp = 0.95;
            base.p = base.p * dp + target_base_location.p * (1.0 - dp);
        }
    }
    ...
    for (size_t l=0, _l=limbs.size(); l<_l; l++) {
        ...
        // is this limb in contact?
        if(limbsInContactItr != contacts.contacts.end() && l == limbsInContactItr->limb) {
            contact = &*limbsInContactItr;
            limbsInContactItr++;
            if(limbsInContactItr != contacts.contacts.end() && limbsInContactItr->limb < l)
                // I dont want to waste cycles searching the limbsInContact list for every limb
                throw Exception::OutOfOrder("Let's be optimal, I need limbs arguments sorted in order!");
        }

        if (limb->options_.model == Limb::Leg) {
            KDL::Frame& foot = state.tf[limb->options_.to_link];
            minZ = std::min(minZ, foot.p.z());

            if(limb->options_.to_link != interactingSegment) {
                // todo: we should ensure one foot is on the floot at least
                if (contact != nullptr && contact->staticFriction > 0.5) {
                    // get foot TF
                    foot.p.z(0);
                    foot.M = heading;
                    feetInSupport++;

#if 0               // contrappose adjustment of hip
                    baseTilt = 0.08 * M_PI;
                    if(limb->options_.to_link == left_foot_link)
                        baseTilt *= -1.;
#endif
                }
            } else {
                // dont let the user pull the foot through the floor
                if(foot.p.z() < 0)
                    foot.p.z(0.0);
            }
        }
    ...
    // compute hunch angle by using standing height and todo: CoM properties of base+backpack
    r = (feetInSupport == 1) ? baseTilt : 0;
    base.M = KDL::Rotation::RPY(r, p, y);

    // todo: we should be smarter about calling compute all the time
    updateIK(state);
    updateState(state);
