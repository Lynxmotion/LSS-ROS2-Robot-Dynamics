...
CustomTreeIkSolverPos_Online::CustomTreeIkSolverPos_Online(unsigned int nr_of_jnts,
...
CustomTreeIkSolverPos_Online::~CustomTreeIkSolverPos_Online()
{}


double CustomTreeIkSolverPos_Online::CartToJnt(const JntArray& q_in, const Frames& p_in, JntArray& q_out)
{
...
double CustomTreeIkSolverPos_Online::CartToJnt_it(const JntArray& q_in, const Frames& p_in, JntArray& q_out)
{
  assert(q_out.rows() == q_in.rows());
...
bool CustomTreeIkSolverPos_Online::enforceCartVelLimits()
{
  // relative overshoot
...
bool CustomTreeIkSolverPos_Online::enforceJointVelLimits()
{
...
void CustomTreeIkSolverPos_Online::enforceCartVelLimits_it(Twist& old_twist, Twist& current_twist)
{
  bool max_exceeded = false;
  double x_dot_trans, x_dot_rot;
...
void CustomTreeIkSolverPos_Online::enforceJointVelLimits_it(JntArray& q_dot_old, JntArray& q_dot_current)
{
  // check, if one (or more) joint velocities exceed the maximum value
  // and if so, safe the biggest overshoot for scaling q_dot_ properly
  // to keep the direction of the velocity vector the same
...
void CustomTreeIkSolverPos_Online::filter(JntArray& q_dot, JntArray& q_out, JntArray& q_out_old)
{
  // deadband filter
